...
export class ProjectsTasksReportComponent {
  ...
  getData(){
    this._proreportsService.GetAllTasksByProjectIdW_whithWotkOrder().subscribe(data=>{
      console.log(data);
      this.data.tasks = new MatTableDataSource(data);
      this.data.tasksTemp=data;
  ...
  CheckDate(event: any){
    if(event!=null)
    {
      ...
      this.getData_S_Date(this.data.filter.DateFrom_P,this.data.filter.DateTo_P);
    }
    else{
  ...
  getData_S(){
    var projectid=this.data.selected??0;
    this._proreportsService.GetAllTasksByProjectIdS_withworkorder(projectid,0,"","").subscribe(data=>{
      console.log(data);
      this.data.tasks = new MatTableDataSource(data);
      this.data.tasksTemp=data;
      this.data.tasks.paginator = this.paginator;
  ...
  getData_S_Date(from: any,to: any){
    this._proreportsService.GetAllTasksByProjectIdS_withworkorder(0,0,from,to).subscribe(data=>{
      console.log(data);
      this.data.tasks = new MatTableDataSource(data);
      this.data.tasksTemp=data;
      this.data.tasks.paginator = this.paginator;
  ...
  applyFilter(event: any) {
    ...
    const tempsource = this.data.tasksTemp.filter(function (d: any) {
      return (d.clientName?.trim().toLowerCase().indexOf(val) !== -1 || !val)
       ...
       || (d.fullTaskDescription?.trim().toLowerCase().indexOf(val) !== -1 || !val)
       || (d.userName?.trim().toLowerCase().indexOf(val) !== -1 || !val);
    });
    this.data.tasks = new MatTableDataSource(tempsource);
    this.data.tasks.paginator = this.paginator;
    this.data.tasks.sort = this.sort;
  }
  fill_ProjectType(){
    this._proreportsService.FillProjectSelect().subscribe(data=>{
      console.log(data);
...
//#region
  ...
  private getDismissReason(reason: any, type?: any): string {
   ...
   {
    if(this.data.tasks.sort)
    {
      this.PrintDataProject=this.data.tasks.sortData(this.data.tasks.filteredData,this.data.tasks.sort);
    }
    else
    {
      this.PrintDataProject=this.data.tasks.data;
    }

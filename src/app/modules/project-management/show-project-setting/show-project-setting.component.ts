...
@Component({
  selector: 'app-show-project-setting',
  templateUrl: './show-project-setting.component.html',
  styleUrls: ['./show-project-setting.component.scss']
})
export class ShowProjectSettingComponent {
  ...
  EditData_setting_Show :any={
    SettingData:[],
    nodeArray:[],
    linkArray:[],
  }

  model2_setting_Show  :any;
  nodeDataArray_setting_Show = [
    { key: 1,loc:"0 0" },
  ...
  ngOnInit(): void {
    ...
    this.nodeDataArray_setting_Show =[];
    this.linkDataArray_setting_Show =[];
    this.model2_setting_Show = new go.GraphLinksModel(this.nodeDataArray_setting_Show, this.linkDataArray_setting_Show);
  ...
  // @ViewChild('viewFull') viewFull: ElementRef<HTMLElement>;

  // triggerFalseClick() {
  //     let el: HTMLElement = this.viewFull.nativeElement;
  //     el.click();
  // }
  ...
  public selectedNode_setting_Show = null;
  public setSelectedNode_setting_Show (node:any) {
    this.selectedNode_setting_Show = node;
  }
  ProjectName_setting :any=null;
    ...
    getProjectSettingEdit_setting (){
      ...
      this.EditData_setting_Show.nodeArray=[];
      this.EditData_setting_Show.linkArray=[];
      this.nodeDataArray_setting_Show =[];
      this.linkDataArray_setting_Show =[];
      ...
      this._projectsettingService.GetAllNodesTasks_Project(ProId).subscribe(res=>{
          this.EditData_setting_Show .SettingData=res;
          console.log("this.EditData_setting_Show.SettingData");
          console.log(this.EditData_setting_Show.SettingData);

          this.EditData_setting_Show.nodeArray.push({ "key": 0, "loc": "-584 -74", "name": projno, "colorfill": "#0fac03" })
          res.nodeDataArray.forEach((r: any) => {
            if (r.nodeLocation != null) {
              if (r.type === 1) {   //mainphase
                this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "loc": r.nodeLocation, "name": r.descriptionAr, isGroup: true, "colorfill": "#70b9dc" })
              }
              else if (r.type === 2) { //subphase
                this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "loc": r.nodeLocation, "name": r.descriptionAr, group: r.parentId, isGroup: true, "colorfill": "#b7e89d"  })
              }
              else {  ///task
                if (r.isTemp == true) {
                  this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "name": r.descriptionAr, group: r.parentId, "toolTipName": r.userName, "colorfill": "#d42d34" })
                }
                else {
                  if (r.execPercentage != null) {
                    if (r.execPercentage == 100) {
                      this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "name": r.descriptionAr, group: r.parentId, "toolTipName": r.userName, "colorfill": "#bfbfbf" })
                    }
                    else {
                      this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "name": r.descriptionAr, group: r.parentId, "toolTipName": r.userName, "colorfill": "#fe9100" })
                    }
                  }
                  else {
                    this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "name": r.descriptionAr, group: r.parentId, "toolTipName": r.userName, "colorfill": "#fe9100" })
              ...
              if (r.type === 1) {  //mainphase
                this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "loc": "50 50", size: "150 50", "name": r.descriptionAr, isGroup: true, "colorfill": "#70b9dc" })
              }
              else if (r.type === 2) { //subphase
                this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "loc": "50 50", size: "150 50", "name": r.descriptionAr, group: r.parentId, isGroup: true, "colorfill": "#b7e89d"  })
              }
              else { ///task
                if (r.isTemp == true) {
                  this.EditData_setting_Show.nodeArray.push({ "key": r.phaseTaskId, "name": r.descriptionAr, group: r.parentId, "toolTipName": r.userName, "colorfill": "#d42d34" })
          ...
          res.firstLevelNode.forEach((r: any) => {
            this.EditData_setting_Show.linkArray.push({ "from": 0, "to": r.phaseTaskId, "name": "" })
          });
          res.linkDataArray.forEach((r: any) => {
            this.EditData_setting_Show.linkArray.push({ "from": r.predecessorId, "to": r.successorId, "name": "" })
          });

          this.nodeDataArray_setting_Show=this.EditData_setting_Show.nodeArray;
          this.linkDataArray_setting_Show=this.EditData_setting_Show.linkArray;

          // console.log("this.nodeDataArray------------ linkDataArray");
          // console.log(this.nodeDataArray_setting_Show);
          // console.log(this.linkDataArray_setting_Show);
    ...
    merge(modal:any) {
      ...
      {
        this.toast.error("من فضلك أكنب اسم المهمة و الملاحظات", 'رسالة');return;
      }
      ...
      if(this.SettingDetailsData.TasksIdArray.length>=2)
      {
        ...
        this.toast.error("من فضلك اختر اكثر من مهمة",'رسالة');
      }
  ...
  //----------------------------------(End)-City Type---------------------------------------------
    ...
    saveProSettings() {
      ...
      {
        this.toast.error("اكتب وصف السير ورقمه", 'رسالة');
        return;
      }
      ...
      for (var i = 0; i < this.linkDataArray_setting_Show.length; i++) {
          if (this.linkDataArray_setting_Show [i].from != 0) {
          var dependencyObj :any = {};
          dependencyObj.DependencyId = 0;
          dependencyObj.PredecessorId = this.linkDataArray_setting_Show [i].from;
          dependencyObj.SuccessorId = this.linkDataArray_setting_Show [i].to;
      ...
      if (LinkArrayList.length == 0) {
          for (var i = 0; i < this.linkDataArray_setting_Show.length; i++) {
              if (this.linkDataArray_setting_Show [i].from == 0) {
                  ...
                  dependencyObj1.PredecessorId = this.linkDataArray_setting_Show [i].to;
                  dependencyObj1.SuccessorId = this.linkDataArray_setting_Show [i].to;
                  LinkArrayList.push(dependencyObj1);
              }
          }
      }
      for (var i = 0; i < this.nodeDataArray_setting_Show .length; i++) {
          if (this.nodeDataArray_setting_Show [i].key != 0) {
              var LocObj :any = {};
              LocObj.LocationId = 0;
              LocObj.SettingId = this.nodeDataArray_setting_Show [i].key;
              LocObj.ProSubTypeId = this.modalDetails.subProjectType;
              LocObj.Location = this.nodeDataArray_setting_Show [i].loc;
  ...
  addNewTask(modal:any) {
    ...
    if (this.AddTaskData.timetypeid == 1 || this.AddTaskData.timetypeid == 2) {
        if (this.AddTaskData.timefrom == null || this.AddTaskData.timeto== null) {
            this.toast.error("من فضلك تأكد من بداية المهمة ونهايتها بالتوقيت", 'رسالة');return;
        }
        else {
        ...
        if (this.AddTaskData.datefrom == null || this.AddTaskData.dateto == null) {
            this.toast.error("من فضلك تأكد من بداية المهمة ونهايتها بالتاريخ", 'رسالة');return;
        }
    }
    this.CalTotalTime();
...
Changeorgisreq() {
  if(this.AddTaskData.taskgoal)
  {
    ...
    {
      this.AddTaskData.taskgoal=false;this.AddTaskData.projectgoalstxt=null;this.AddTaskData.projectgoals=null;
      this.toast.error("من فضلك أختر مشروع أولا", 'رسالة'); return;
    }
  }
  else
  {
  ...
  SaveProSettingFun() {
    ...
    for (var i = 0; i < this.nodeDataArray_setting_Show.length; i++) {
        if (this.nodeDataArray_setting_Show[i].key != 0) {
            var LocObj:any = {};
            LocObj.LocationId = 0;
            LocObj.TaskId = this.nodeDataArray_setting_Show[i].key;

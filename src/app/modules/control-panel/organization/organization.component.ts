...
export class OrganizationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.MessageServiceProvidersList = [
      ...
      { id: 2, Name: 'فور جوالي' },
      { id: 3, Name: 'مسجات' },
      { id: 4, Name: 'بوابة الرسائل' },
      { id: 5, Name: 'Twilio' },
    ];
    this.WhatsAppServiceProvidersList = [
      { id: "twilio", Name: 'Twilio' },
      { id: "ultramsg", Name: 'UltraMsg' },
    ];
    this.users = [
  ...
  FillSelectEmployeeWorkers() {
    this.organizationService.FillEmployeeSelect().subscribe((data) => {
  ...
  Getemployeebyid() {
    if (
      this.mainInfoForm.controls['representorEmpId'].value != null &&
      this.mainInfoForm.controls['representorEmpId'].value != 0
    ) {
      this.organizationService
        .GetEmployeeById(this.mainInfoForm.controls['representorEmpId'].value)
        .subscribe((data) => {
          if (data.result.dawamId == 0) {
            this.toast.error('يرجي ضبط دوام الموظف', 'رسالة');
          } else {
            this.mainInfoForm.controls['as'].setValue(data.result.jobName);
            this.mainInfoForm.controls['NationalId'].setValue(
              data.result.nationalId
            );
          }
        });
    }
  ...
  SaveCity() {
    ...
    this.organizationService.SaveCity(this._cityobj).subscribe(
      (result: any) => {
        if (result.statusCode == 200) {
          this.FillCitySelect();
          this.getAllCities();
  ...
  getcityName() {
    if (this.nationalitysourceselected.length > 0) {
      this.nationalitysourceselected.forEach((element: any) => {
        if (this.mainInfoForm.controls['cityId'].value == element.id) {
          if (this.lang == 'ar') {
            this.mainInfoForm.controls['cityName'].setValue(element.name);
          } else {
            this.mainInfoForm.controls['cityName'].setValue(element.nameEn);
          }
        }
      });
    }
  ...
  SavesmsSetting() {
    const prames = {
      ApiUrl: this.SavesmsSettingForm.controls['apiUrl'].value,
      Password: this.SavesmsSettingForm.controls['bearerTokens'].value,
      // messageServiceProviders: this.SavesmsSettingForm.controls['messageServiceProviders'].value,
      SettingId: this.SavesmsSettingForm.controls['SettingId'].value??0,
      SenderName: this.SavesmsSettingForm.controls['senderNamesms'].value,
      MobileNo: this.SavesmsSettingForm.controls['phone'].value,
      UserName: this.SavesmsSettingForm.controls['username'].value,
      SendCustomerSMS:
        this.SavesmsSettingForm.controls['SendCustomerSMS'].value,
    };
    this.organizationService.SavesmsSetting(prames).subscribe(
      (result: any) => {
        if (result.statusCode == 200) {
          this.toast.success(
            this.translate.instant(result.reasonPhrase),
  ...
  SaveWhatsAppSetting() {
    debugger
    const prames = {
      MobileNo: this.SaveWhatsAppSettingForm.controls['phoneW'].value,
      InstanceId: this.SaveWhatsAppSettingForm.controls['InstanceId'].value,
      Token: this.SaveWhatsAppSettingForm.controls['Token'].value,
      Password: this.SaveWhatsAppSettingForm.controls['Token'].value,
      TypeName: this.SaveWhatsAppSettingForm.controls['WhatsAppServiceProviders'].value,
      SettingId: this.SaveWhatsAppSettingForm.controls['SettingId'].value??0,
      UserName: this.SaveWhatsAppSettingForm.controls['usernameW'].value,
      SenderName: this.SaveWhatsAppSettingForm.controls['SenderNameW'].value,
      Sendactivation:this.SaveWhatsAppSettingForm.controls['Sendactivation'].value,
      SendactivationOffer:this.SaveWhatsAppSettingForm.controls['SendactivationOffer'].value,
      SendactivationProject:this.SaveWhatsAppSettingForm.controls['SendactivationProject'].value,
      SendactivationSupervision:this.SaveWhatsAppSettingForm.controls['SendactivationSupervision'].value,
    };
    this.organizationService.SaveWhatsAppSetting(prames).subscribe((result: any) => {
        if (result.statusCode == 200) {
  ...
  SaveAttDeviceSetting() {
    const prames = {
      AttDeviceSittingId:
        this.saveEmailSettingForm.controls['AttDeviceSittingId'].value,
      ArgCompanyCode:
        this.saveEmailSettingForm.controls['CompanyCodedevices'].value,
      ArgDeviceName:
        this.saveEmailSettingForm.controls['DeviceNamedevices'].value,
      ArgEmpUsername:
        this.saveEmailSettingForm.controls['usernamedevices'].value,
      ArgEmpPassowrd:
        this.saveEmailSettingForm.controls['passworddevices'].value,
    };
    this.organizationService.SaveAttDeviceSetting(prames).subscribe(
      (result: any) => {
        if (result.statusCode == 200) {
          this.toast.success(
            this.translate.instant(result.reasonPhrase),
  ...
  savesupportmsg() {
    const prames = {
      OrganizationId: this.mainInfoForm.controls['organizationId'].value,
      SupportMessageAr: this.mainInfoForm.controls['supportMessageAr'].value,
      SupportMessageEn: this.mainInfoForm.controls['supportMessageEn'].value,
    };
    this.organizationService.savesupportmsg(prames).subscribe(
      (result: any) => {
        if (result.statusCode == 200) {
          this.toast.success(
            this.translate.instant(result.reasonPhrase),
  ...
  GetsmsSetting() {
    this.organizationService.GetsmsSetting().subscribe((data: any) => {
      this.SavesmsSettingForm.controls['SettingId'].setValue(
        data?.result?.settingId
      );
      this.SavesmsSettingForm.controls['apiUrl'].setValue(data?.result?.apiUrl);
      this.SavesmsSettingForm.controls['bearerTokens'].setValue(
        data?.result?.password
      );
      this.SavesmsSettingForm.controls['senderNamesms'].setValue(
        data?.result?.senderName
      );
      this.SavesmsSettingForm.controls['phone'].setValue(
        data?.result?.mobileNo
      );
      this.SavesmsSettingForm.controls['username'].setValue(
        data?.result?.userName
      );
      this.SavesmsSettingForm.controls['SendCustomerSMS'].setValue(
        data?.result?.sendCustomerSMS
      );
    });


    this.organizationService.GetWhatsAppSetting().subscribe((data: any) => {
      this.SaveWhatsAppSettingForm.controls['SettingId'].setValue(data?.result?.settingId);
      this.SaveWhatsAppSettingForm.controls['WhatsAppServiceProviders'].setValue(data?.result?.typeName);
      this.SaveWhatsAppSettingForm.controls['phoneW'].setValue(data?.result?.mobileNo);
      this.SaveWhatsAppSettingForm.controls['InstanceId'].setValue(data?.result?.instanceId);
      this.SaveWhatsAppSettingForm.controls['Token'].setValue(data?.result?.token);
      this.SaveWhatsAppSettingForm.controls['usernameW'].setValue(data?.result?.userName);
      this.SaveWhatsAppSettingForm.controls['SenderNameW'].setValue(data?.result?.senderName);
      this.SaveWhatsAppSettingForm.controls['Sendactivation'].setValue(data?.result?.sendactivation);
      this.SaveWhatsAppSettingForm.controls['SendactivationOffer'].setValue(data?.result?.sendactivationOffer);
      this.SaveWhatsAppSettingForm.controls['SendactivationProject'].setValue(data?.result?.sendactivationProject);
      this.SaveWhatsAppSettingForm.controls['SendactivationSupervision'].setValue(data?.result?.sendactivationSupervision);
    });


    this.organizationService.GetEmailSetting().subscribe((data: any) => {
      this.saveEmailSettingForm.controls['SettingId'].setValue(
        data?.result?.settingId
      );
      this.saveEmailSettingForm.controls['email'].setValue(
        data?.result?.senderEmail
      );
      this.saveEmailSettingForm.controls['password'].setValue(
        data?.result?.password
      );
      this.saveEmailSettingForm.controls['senderName'].setValue(
        data?.result?.senderName
      );
      this.saveEmailSettingForm.controls['host'].setValue(data?.result?.host);
      this.saveEmailSettingForm.controls['port'].setValue(data?.result?.port);
      this.saveEmailSettingForm.controls['ssl'].setValue(data?.result?.ssl);
      this.saveEmailSettingForm.controls['DisplayName'].setValue(
        data?.result?.displayName
      );
    });

    this.organizationService.GetAttDeviceSetting().subscribe((data: any) => {
      this.saveEmailSettingForm.controls['AttDeviceSittingId'].setValue(
        data?.result?.attDeviceSittingId
      );
      this.saveEmailSettingForm.controls['CompanyCodedevices'].setValue(
        data?.result?.argCompanyCode
      );
      this.saveEmailSettingForm.controls['DeviceNamedevices'].setValue(
        data?.result?.argDeviceName
      );
      this.saveEmailSettingForm.controls['usernamedevices'].setValue(
        data?.result?.argEmpUsername
      );
      this.saveEmailSettingForm.controls['passworddevices'].setValue(
        data?.result?.argEmpPassowrd
      );
    });

    this.organizationService.GetComDomainLink_Org().subscribe((data: any) => {
      this.mainInfoForm.controls['organizationId'].setValue(
        data?.result?.organizationId
      );
      this.mainInfoForm.controls['comDomainLink'].setValue(
        data?.result?.comDomainLink
      );
      this.mainInfoForm.controls['comDomainAddress'].setValue(
        data?.result?.comDomainAddress
      );
      this.mainInfoForm.controls['apiBaseUri'].setValue(
        data?.result?.apiBaseUri
      );
      this.mainInfoForm.controls['tameerAPIURL'].setValue(
        data?.result?.tameerAPIURL
      );
    });
    this.organizationService
      .GetApplicationVersion_Org()
      .subscribe((data: any) => {
        this.mainInfoForm.controls['lastversionIOS'].setValue(
          data?.result?.lastversionIOS
        );
        this.mainInfoForm.controls['lastvesionAndroid'].setValue(
          data?.result?.lastvesionAndroid
        );
        this.mainInfoForm.controls['messageUpdateAr'].setValue(
          data?.result?.messageUpdateAr
        );
        this.mainInfoForm.controls['messageUpdateEn'].setValue(
          data?.result?.messageUpdateEn
        );
        this.mainInfoForm.controls['supportMessageAr'].setValue(
          data?.result?.supportMessageAr
        );
        this.mainInfoForm.controls['supportMessageEn'].setValue(
          data?.result?.supportMessageEn
        );
      });
  }
  ...
  SaveEmailSettingSMS() {
    if (this.confirmepasswordSetting == true) {
      const prames = {
        // organizationId: this.mainInfoForm.controls['organizationId'].value,
        // email: this.saveEmailSettingForm.controls['email'].value,
        // password: this.saveEmailSettingForm.controls['password'].value,
        // senderName: this.saveEmailSettingForm.controls['senderName'].value,
        // port: this.saveEmailSettingForm.controls['port'].value,
        // host: this.saveEmailSettingForm.controls['host'].value,
        // ssl: this.saveEmailSettingForm.controls['ssl'].value,
        // DisplayName: this.saveEmailSettingForm.controls['DisplayName'].value,
        // SettingId:0,

        SettingId: this.saveEmailSettingForm.controls['SettingId'].value,
        SenderEmail: this.saveEmailSettingForm.controls['email'].value,
        Password: this.saveEmailSettingForm.controls['password'].value,
        SenderName: this.saveEmailSettingForm.controls['senderName'].value,
        Host: this.saveEmailSettingForm.controls['host'].value,
        Port: this.saveEmailSettingForm.controls['port'].value,
        SSL: this.saveEmailSettingForm.controls['ssl'].value,
        UserId: this.userG.userId,
        BranchId: this.userG.branchId,
        DisplayName: this.saveEmailSettingForm.controls['DisplayName'].value,
      };
      this.organizationService
        .SaveEmailSetting(prames)
        .subscribe((result: any) => {
          if (result.statusCode == 200) {
            this.toast.success(
              this.translate.instant(result.reasonPhrase),

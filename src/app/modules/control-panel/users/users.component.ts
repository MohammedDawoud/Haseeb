...
export class UsersComponent implements OnInit {
  ...
  forwardProjectselectedprojectId: any = null;
  projectId: any = null;
  ConvertManagerProjects(modal: any) {
    if (this.listforwardProjectprojectNo == true) {
      this._sysServicesService
        .ConvertMoreManagerProjects(
          this.selectedProjectUser,
          this.forwardProjectselectedprojectId,
          this.projectId
        )
        .subscribe((result: any) => {
          if (result.statusCode == 200) {
            ...
            this.projectId = null;
            this.forwardProjectselectedprojectId = null;
            this.listforwardProjectprojectNo = false;
            this.onSelectProjectUser();
            modal.dismiss();
          } else {
            this.toast.error(
              this.translate.instant(result.reasonPhrase),
  ...
  ProjectUserIdList: any;
  onSelectProjectUser() {
    this.selection.projects.clear();

    if (this.selectedProjectUser) {
      var _projectVM = new ProjectVM();
      _projectVM.mangerId = this.selectedProjectUser;
      _projectVM.status = 0;
      var obj = _projectVM;
      this._sysServicesService.GetProjectsSearch(obj).subscribe((data) => {
        this.ProjectUserIdList = data;
        console.log(data);
        this.modals.rows.projects = new MatTableDataSource(data);
        this.modals.rows.projects.paginator = this.projectsTablepag;
        this.modals.rows.projects.sort = this.projectsTable;
  ...
  applyFilterProjectd(event: any) {
    const val = event.target.value.toLowerCase();
    this.ProjectUserIdList.forEach((element: any) => {
      if (element.projectTypeName == null) {
        element.projectTypeName = '';
      }
    });
    const tempsource = this.ProjectUserIdList.filter(function (d: any) {
      return (
        d.projectNumber.toString()?.trim().toLowerCase().indexOf(val) !== -1 ||
        !val ||
        d.clientName.toString()?.trim().toLowerCase().indexOf(val) !== -1 ||
        !val ||
        d.projectTypeName.toString()?.trim().toLowerCase().indexOf(val) !==
          -1 ||
        !val
      );
    });

    this.modals.rows.projects = new MatTableDataSource(tempsource);
    this.modals.rows.projects.paginator = this.paginator;
    this.modals.rows.projects.sort = this.sort;
  }
  ...
  onSelectFillPriv() {
    if (this.selectedFillPrivId) {
      this._sysServicesService
        .getUsersByPrivilegesIds(this.selectedFillPrivId)
        .subscribe(
          (reportData: any) => {
            // Process the privilege report data here
            this.ProjectUserIdList = reportData;

            this.modals.rows.permissions = new MatTableDataSource(reportData);
            this.modals.rows.permissions.paginator = this.projectsTablepag;
            this.modals.rows.permissions.sort = this.projectsTable;
  ...
  open(content: any, data: any, size: any, positions?: any, modalType?: any) {
    ...
    } else if (modalType == 'forwardProjectprojectNo') {
      this.projectId = null;
      this.projectId = data;
      this.listforwardProjectprojectNo = false;
    } else if (modalType == 'listforwardProjectprojectNo') {
      this.projectId = [];
      this.selection.projects.selected.forEach((item) => {
        this.projectId.push(item.projectId);
      });
      this.listforwardProjectprojectNo = true;

...
@Component({
  selector: 'app-monthlyinvoiceswithdayes',
  templateUrl: './monthlyinvoiceswithdayes.component.html',
  styleUrls: ['./monthlyinvoiceswithdayes.component.scss']
})
export class MonthlyinvoiceswithdayesComponent {
showTable : boolean = false ;
  ...
  title: any = {
    ...
    sub: {
      ar:' متابعة المبيعات  والمردو الشهري',
      en:'Follow Up Monthly Payments and Earns and Mardod',
    },
  };
 ...
 GetOrganizationData() {
  ...
  onMonthSelect(date: Date): void {
    ...
    // Refresh your data with the new filter dates
    this.RefreshData();
  }
  ...
  RefreshData(){
    ...
    {
      formData.append('PayType',this.data.filter.customerId);
    }
    if(this.data.filter.startdate!=null )
    {
      formData.append('StartDate',this._sharedService.date_TO_String(this.data.filter.startdate));

    }
    if(this.data.filter.enddate!=null )
    {
      formData.append('EndDate',this._sharedService.date_TO_String(this.data.filter.enddate));

    }

    this._accountsreportsService.GetMonthlypaymentsandearns_WithDayes(formData).subscribe(data => {
      debugger
  ...
  x.push({
        'اليوم': this.projectsDataSourcedata[index].invoiceDate,
        ...
        'الربح النهائي': this.projectsDataSourcedata[index].finalEarnings,

      })
    
    }
    this.lang == "ar" ? this._accountsreportsService.customExportExcel(x, " المبيعات والارباح الشهري") :
      this._accountsreportsService.customExportExcel(x, "Monthly Payment and Earnings");
  }

...
@Component({
  selector: 'app-yearlyinvoiceswit-months',
  templateUrl: './yearlyinvoiceswit-months.component.html',
  styleUrls: ['./yearlyinvoiceswit-months.component.scss']
})
export class YearlyinvoiceswitMonthsComponent {
showTable : boolean = false ;
  ...
  title: any = {
    ...
    sub: {
      ar:' متابعة ارباح المبيعات  والمردو السنوي',
      en:'Follow Up Yearly Payments and Earns and Mardod',
    },
  };
 ...
 GetOrganizationData() {
  ...
  onMonthSelect(date: Date): void {
    ...
    this.data.filter.dateint = date.getFullYear();
  ...
  RefreshData(){
    ...
    if(this.data.filter.dateint!=null )
    {
      formData.append('Year',this.data.filter.dateint);

    }


    this._accountsreportsService.GetyearlyInvoicesWithDetails_ByYear(formData).subscribe(data => {
      debugger
  ...
  exportData() {
    ...
    this.lang == "ar" ? this._accountsreportsService.customExportExcel(x, " المبيعات والارباح السنوي") :
      this._accountsreportsService.customExportExcel(x, "Yearly Payment and Earnings");
  }

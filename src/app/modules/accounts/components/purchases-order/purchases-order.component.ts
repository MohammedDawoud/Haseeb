...
@Component({
  ...
  styleUrls: ['./purchases-order.component.scss'],
  providers: [DatePipe],
})
...
// data in runningTasksModal
  ...
  InvoicessDataSource: MatTableDataSource<any> = new MatTableDataSource([{}]);
  userG: any = {};
  constructor(
  ...
  RefreshData() {
    var _voucherFilterVM = new VoucherFilterVM();
    _voucherFilterVM.type = this.dataInvoice.filter.Type;
    _voucherFilterVM.voucherNo = this.dataInvoice.filter.search_InvoiceNumber ??0;
  ...
  getColorStatus(element: any) {
    if (element?.purchaseOrderNo !=null && element?.purchaseOrderNo != '') {
      return '#30b550';
    } else if (element?.purchaseOrderNo == null || element?.purchaseOrderNo == '') {
      return '#c51313';
    } else {
      return '#000000';
    }
  ...
  GenerateVoucherNumber() {
    this._invoiceService.GenerateVoucherNumber(this.modalInvoice.Type)
      .subscribe((data) => {
  ...
  FillCustAccountsSelect2AndUpdate_Save(PayType: any,modal:any) {
    if (PayType) {
      this._invoiceService
        .FillCustAccountsSelect2(PayType)
        .subscribe((data) => {
  ...
  checkPayTypeAndSave(modal:any) {
    ...
    if (+remainder < 1 && +_paidValue > 0) {
      ...
      if (this.modalInvoice.PayType == 8) {
        ...
        this.FillCustAccountsSelect2AndUpdate_Save(1,modal);
      } else {
        this.saveInvoice(modal);
      }
    } else if (
      ...
      if (this.modalInvoice.PayType != 8) {
        ...
        this.FillSuppAccountsSelect_Save(8,modal);
      } else {
        this.saveInvoice(modal);
      }
    } else {
  ...
  FillSuppAccountsSelect_Save(PayType: any,modal:any) {
    if (PayType) {
      this.purchasesBillService
        .FillSuppAccountsSelect(PayType)
        .subscribe((data) => {
  ...
  saveInvoice(modal:any) {
    ...
    if (this.modalInvoice.WhichClick == 1) {
      this.purchasesBillService.SaveandPostPurchaseOrderForServices(VoucherObj).pipe(take(1)).subscribe(
        (result: any) => {
          if (result.statusCode == 200) {
            this.toast.success(
              this.translate.instant(result.reasonPhrase),
      ...
      this.purchasesBillService
        .SaveandPostPurchaseOrderForServices(VoucherObj).pipe(take(1))
        .subscribe(
          (result: any) => {
            if (result.statusCode == 200) {
              this.toast.success(
  ...
  GetAllJournalsByInvIDPurchase(invid: any) {
    this._invoiceService
      .GetAllJournalsByInvIDPurchaseOrder(invid)
      .subscribe((data) => {
  ...
  ConverOrderToInvoice() {
    debugger
    var invid=this.RowInvoiceData.invoiceId;

    this.purchasesBillService.ConverOrderToInvoice(invid)
      .subscribe((result: any) => {
        if (result.statusCode == 200) {
          this.toast.success(
            this.translate.instant(result.reasonPhrase),
  ...
  DeletePurchaseOrder(modal:any) {
    debugger
    var invid=this.RowInvoiceData.invoiceId;

    this._invoiceService.DeleteVoucher(invid)
      .subscribe((result: any) => {
        if (result.statusCode == 200) {
          this.toast.success(
            this.translate.instant(result.reasonPhrase),
  ...
  exportData() {
    let x = [];
    for (let index = 0; index < this.InvoicessDataSource.data.length; index++) {
      x.push({
        'حالة الطلب': this.InvoicessDataSource.data[index].purchaseOrderStatus,
        'نوع الدفع': this.InvoicessDataSource.data[index].payTypeName,
        ' صافي الدخل ': this.InvoicessDataSource.data[index].totalValue,
        'تاريخ أمر الشراء': this.InvoicessDataSource.data[index].date,
        'رقم أمر الشراء': this.InvoicessDataSource.data[index].invoiceNumber,
      })
    }

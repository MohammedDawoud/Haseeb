...
export class EntryvoucherService {
  ...
  GetAllJournalsByDailyID(invId: any) {
    var url = `${environment.apiEndPoint}DailyJournal/GetAllJournalsByDailyID?&invId=${invId}`;
    return this.http.get<any>(url);
  }
  ...
  SaveDailyVoucher(obj:any): Observable<any> {
    const headers = { 'content-type': 'application/json'}
    const body=JSON.stringify(obj);
    return this.http.post(this.apiEndPoint + 'Voucher/SaveDailyVoucher', body,{'headers':headers});
  }
